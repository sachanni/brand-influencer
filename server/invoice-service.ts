...
export class InvoiceService {
  ...
  static determineTaxRegion(user: User): string {
    ...
    // Default to India (18% GST) for most users
    // This matches the common use case and provides proper tax calculation
    return 'IN';
  }
  ...
  static async generateMilestonePayments(invoice: Invoice): Promise<PaymentMilestone[]> {
    ...
    // Standard milestone structure: 50% upfront, 30% content delivery, 20% completion
    const milestoneStructure = [
      {
        number: 1,
        type: 'upfront' as const,
        description: 'Upfront Payment - Campaign Initiation',
        percentage: 50,
        triggerCondition: 'campaign_approval',
        requirements: 'Campaign proposal acceptance and contract signing'
      },
      {
        number: 2,
        type: 'content_delivery' as const,
        description: 'Content Delivery Payment',
        percentage: 30,
        triggerCondition: 'content_submission',
        requirements: 'Delivery of agreed content according to campaign specifications'
      },
      {
        number: 3,
        ...
        requirements: 'Campaign completion and performance metrics achievement'
      }
    ];
